...
export const GoodCard = ({
  ...
  return (
    <article id={id} className={`${styles.content} ${extraClass}`} onClick={onClick}>
      <Link to={`/gift/${id}`} className={styles.img_box}>
        <img className={styles.img} src={img} alt='Фото товара.' />
      </Link>
      <div className={styles.data_box}>
        <p className={`text text_type_h2 text_color_primary mb-4 ${styles.price}`}>
          {`${price} руб.`}
        </p>
        <p className={`text text_type_main text_color_primary mb-10 ${styles.name}`}>{name}</p>
